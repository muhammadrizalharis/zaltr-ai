...
/** Pengingat masa aktif paket segera berakhir (mis. H-3). */
export function expiryReminderEmail(p: {
  ...
  const halo = p.name ? `Halo ${p.name},` : "Halo,";
  const tgl = p.expiresAt.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" });
  const sisa = `${p.daysLeft} hari lagi`;
  const subject = `Paket ${p.planLabel} berakhir ${sisa} — calyzr.ai`;
  const html = `<!DOCTYPE html><html><body style="margin:0;background:#f1f5f9;padding:24px;font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif">
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0">
    <div style="background:linear-gradient(135deg,#f59e0b,#ef4444);padding:24px">
      <div style="color:#fff;font-size:20px;font-weight:800;letter-spacing:.3px">CALYZR.AI</div>
      <div style="color:#fff5e6;font-size:13px;margin-top:2px">Pengingat masa aktif</div>
    </div>
    <div style="padding:24px">
      <p style="margin:0 0 4px;font-size:15px;color:#0f172a">${halo}</p>
      <p style="margin:0 0 16px;font-size:15px;color:#0f172a">Paket <b>${p.planLabel}</b> kamu berakhir <b style="color:#b45309">${tgl}</b> (${sisa}). Setelah itu, sisa kredit (${p.balance.toLocaleString("id-ID")}) hangus dan model premium tertutup.</p>
      <p style="margin:0 0 16px;font-size:14px;color:#334155">Perpanjang sebelum tanggal itu agar akunmu tetap aktif tanpa jeda.</p>
      <a href="${p.appUrl}/chat/upgrade" style="display:inline-block;background:linear-gradient(135deg,#f59e0b,#ef4444);color:#fff;text-decoration:none;padding:11px 20px;border-radius:12px;font-weight:600;font-size:14px">Perpanjang paket →</a>
      <p style="margin:18px 0 0;font-size:12px;color:#94a3b8">Abaikan email ini jika kamu memang tidak ingin memperpanjang.</p>
    </div>
  </div></body></html>`;
  const text =
    `${halo}\n\nPaket ${p.planLabel} kamu berakhir ${tgl} (${sisa}). Setelah itu sisa kredit (${p.balance.toLocaleString("id-ID")}) hangus dan model premium tertutup.\n` +
    `Perpanjang: ${p.appUrl}/chat/upgrade`;
  return { subject, html, text };
}
